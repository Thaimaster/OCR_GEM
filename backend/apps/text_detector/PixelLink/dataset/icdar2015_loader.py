...
mp_diff_imgs_paths = multiprocessing.Manager().dict()

# ic15_root_dir = '/home/gem/phucph/PixelLink.pytorch/dataset/'
ic15_root_dir = 'dataset/'
#train image include 2 folder:  train_images & japan_images
ic15_train_data_dir = ic15_root_dir + 'train_images/'
diff_train_data_dir = ic15_root_dir + 'japan_images/'
...
def load_data_gt(data_dirs, gt_dirs):
...
class IC15Loader(data.Dataset):
    ...
    # difficult_imgs_paths_global = set()
    def __init__(self, is_transform=False, img_size=None, ic15_train_data_dir = ic15_train_data_dir):
        self.is_transform = is_transform
        
        self.img_size = img_size if (img_size is None or isinstance(img_size, tuple)) else (img_size, img_size)

        data_dirs = [ic15_train_data_dir]
        data_diff_dirs = [diff_train_data_dir]
    ...
    def add_difficult_images(self, values):
        self.difficult_imgs_paths.add(values)

    def remove_difficult_images(self, values):
        self.difficult_imgs_paths.remove(values)

    def get_mp_diff_img_path(self):
        with lock:
        #   print("get item",len(mp_diff_imgs_paths))
          if len(mp_diff_imgs_paths) != 0 :
              img_path = random.choice(mp_diff_imgs_paths.keys())
              mp_diff_imgs_paths.pop(img_path)
            #   print("after pop",mp_diff_imgs_paths)
              return img_path
        return None
    
    def get_img_gt(self, index):
        ...
        if random_truth:
          if (torch.rand(1).item() < 0.5):
            ...
            i_path = self.get_mp_diff_img_path()
            if i_path is not None:
              img_path = i_path
              image_name = img_path.split('/')[-1]
              gt_path= ic15_train_gt_dir + image_name.replace(".jpg",".txt") 
    ...
    def __getitem__(self, index):
        img_path, gt_path = self.get_img_gt(index)
...
def test():
    ...
    for i in range(10):
        img,img_paths, cls_label,  cls_weight,  link_label,  link_weight=dataloader.__getitem__(i)
