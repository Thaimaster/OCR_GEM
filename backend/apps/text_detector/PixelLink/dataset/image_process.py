...
def random_resize(image, boxes=None, tags=None, longer_sides=np.arange(2176, 2480, 1)):
    ...
    ratio = longer_side*1.0/max(ori_h,ori_w)
    boxes[:, :, 1] *= ratio
    boxes[:, :, 0] *= ratio
...
# def random_crop_new(image,boxes=None,tags=None, crop_size=(2176, 2176),max_tries=50):
#     crop_h, crop_w = crop_size
#     ori_h, ori_w, c = image.shape
#     print("a",image.shape)
...
#     else:
#         # print("2")
#         scale_h = crop_h * 1.0 / ori_h
#         scale_w = crop_w * 1.0 / ori_w
#         cropped_image = cv2.resize(image, dsize=None, fx=scale_w, fy=scale_h)
