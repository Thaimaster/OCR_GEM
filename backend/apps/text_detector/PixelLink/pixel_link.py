...
def decode_image(pixel_scores, link_scores, 
    ...
    if config.decode_method == DECODE_METHOD_join:
        ...
        mask =  decode_image_by_join(pixel_scores, link_scores, 
                 pixel_conf_threshold, link_conf_threshold)
        print("finish decode img", time.time()-start)
        return mask
...
# @util.dec.print_calling_in_short
# @util.dec.timeit
def mask_to_bboxes(mask, scale_img, image_shape = None, min_area = None,
                   min_height = None, min_aspect_ratio = None):
    import config
    feed_shape = config.train_image_shape
    # print("feed _sahpe", feed_shape)
    ...
    # mask = util.img.resize(img = mask, size = (2176, 2176), 
    ...
    scale_w = mask_w/long_size
    scale_h = mask_h/long_size
    bboxes[:, :, 0] /= scale_h
    bboxes[:, :, 1] /= scale_w
    if scale_img < 1:
        bboxes /= scale_img
        bboxes = np.asarray(bboxes)
    bboxes = [np.concatenate(box).ravel().astype(np.int) for box in bboxes]
