...
def to_bboxes(image_data, scale, pixel_pos_scores, link_pos_scores):
    link_pos_scores=np.transpose(link_pos_scores,(0,2,3,1))
    t1 = time.time()
    mask = decode_batch(pixel_pos_scores, link_pos_scores)[0, ...]
    print("time decode", time.time()-t1)
    t2 = time.time()
    bboxes = mask_to_bboxes(mask, scale, image_data.shape)
    print("time mask", time.time()-t2)
    return mask,bboxes
...
def test(args):
    ...
    if args.arch == "resnet50":
        ...
        model = models.vgg16(pretrained=False,num_classes=18)
        # model = models.ModelParallelVGG16(pretrained=False,num_classes=18) # graph 2 GPU
    ...
    if args.resume is not None:                                         
        if os.path.isfile(args.resume):
            ...
            # 
            # print("md :" , list(model.state_dict().keys())[:10])
            # exit()
            checkpoint = torch.load(args.resume)
            print("bf :" , list(checkpoint['state_dict'].keys())[:10])
            # checkpoint_dict = {}
            # # model graph 2GPU
            # for k,v in checkpoint['state_dict'].items(): 
            #   checkpoint_dict[k.replace('module1', "backbone")] = v
            #   checkpoint_dict[k.replace('module2', "backbone")] = v
            # model.load_state_dict(checkpoint_dict,strict=False)
            
            # print("bf :" , list(checkpoint_dict.keys())[:10])
            print("md :" , list(model.state_dict().keys())[:10])
    ...
    for idx, (org_img,scaled_padded_image, scaled_img, _scale) in enumerate(test_loader):
        print(('progress: %d / %d'%(idx, len(test_loader))))
        sys.stdout.flush()

        img = scaled_padded_image.to(device)
        ...
        debug(idx, data_loader.img_paths, [[text_box,score_s],[link_score,mask]], 'outputs/vis_170/')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale', nargs='?', type=int, default=1,
                        help='Path to previous saved model to restart from')
    parser.add_argument('--long_size', nargs='?', type=int, default=2480,#2480.1280
                        help='Path to previous saved model to restart from')
